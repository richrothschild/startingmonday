...
async function main() {
  ...
  for (const file of csvFiles) {
    ...
    for (const row of rows) {
      ...
      if (!checkSubject(subj)) {
        failures++
        console.log(`[${file}] ${row.full_name || row.email || 'row'}: Subject not compliant`)
      }
    }
  }
  if (failures) {
    console.error(`\nFAIL: ${failures} of ${total} outreach rows have non-compliant subject.`)
    process.exit(1)
  } else {
    console.log(`PASS: All ${total} outreach rows have compliant subject.`)
  }
}
