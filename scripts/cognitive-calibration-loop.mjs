...
const deterministicPath = path.join(process.cwd(), 'docs', 'status', 'cognitive-load.latest.json')
const auditorPath = path.join(process.cwd(), 'docs', 'status', 'cognitive-fluency-auditor.latest.json')
const reportJsonPath = path.join(process.cwd(), 'docs', 'status', 'cognitive-calibration-loop.latest.json')
const reportMdPath = path.join(process.cwd(), 'docs', 'status', 'cognitive-calibration-loop.latest.md')
...
function loadDeterministicSnapshot() {
  ...
  if (!fs.existsSync(deterministicPath)) {
    throw new Error(`Missing deterministic cognitive snapshot: ${deterministicPath}`)
  }
  ...
  for (const page of raw.pages ?? []) {
    ...
    byRoute.set(route, {
      ...
      fluencyScore: Number.isFinite(page.fluency?.score) ? page.fluency.score : null,
...
function loadAuditorSnapshot() {
  ...
  for (const row of sourceRoutes) {
    ...
    byRoute.set(route, {
      route,
      grade: normalizeGrade(row.grade ?? row.fluencyGrade ?? row.cognitiveGrade),
...
function compareSnapshots(deterministic, auditor) {
  ...
  const withinOneGrade = overlap.filter((row) => row.distance !== null && row.distance <= 1).length
  const deterministicStricter = overlap.filter((row) => row.relation === 'deterministic-stricter').length
  const auditorStricter = overlap.filter((row) => row.relation === 'auditor-stricter').length
  ...
  return {
    ...
    summary: {
      ...
      auditorStricter,
      exactAgreementRate: overlap.length === 0 ? null : Number((exact / overlap.length).toFixed(3)),
      withinOneGradeRate: overlap.length === 0 ? null : Number((withinOneGrade / overlap.length).toFixed(3)),
    },
  }
}
...
function buildMarkdown(report) {
  ...
  lines.push(`Deterministic snapshot: ${report.deterministic.generatedAt ?? 'n/a'}`)
  ...
  lines.push(`- Deterministic stricter: ${report.summary.deterministicStricter}`)
  lines.push(`- Auditor stricter: ${report.summary.auditorStricter}`)
  lines.push(`- Exact agreement rate: ${report.summary.exactAgreementRate ?? 'n/a'}`)
  lines.push(`- Within one grade rate: ${report.summary.withinOneGradeRate ?? 'n/a'}`)
  lines.push('')

  if (report.deterministicOnly.length > 0) {
    lines.push('## Deterministic-only routes')
    lines.push('')
    for (const route of report.deterministicOnly.slice(0, 20)) lines.push(`- ${route}`)
    lines.push('')
  }
  ...
  if (report.auditorOnly.length > 0) {
    ...
    lines.push('')
    for (const route of report.auditorOnly.slice(0, 20)) lines.push(`- ${route}`)
    lines.push('')
  }
  ...
  if (disagreements.length === 0) {
    ...
    for (const row of disagreements) {
      lines.push(`- ${row.route}: deterministic=${row.deterministicGrade ?? 'n/a'}, auditor=${row.auditorGrade ?? 'n/a'}, relation=${row.relation}, distance=${row.distance}`)
    }
  }
  lines.push('')
  return `${lines.join('\n')}\n`
...
function buildSlackText(report) {
  if (report.status === 'blocked-missing-auditor-snapshot') {
    return [
      ...
      '',
      '*Action needed*',
      '- Provide docs/status/cognitive-fluency-auditor.latest.json from Page Experience Auditor output, then re-run this workflow.',
    ].join('\n')
  }
...
async function main() {
  ...
  console.log(`Cognitive calibration loop completed with status=${report.status}.`)
}
