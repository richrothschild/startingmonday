...
const reportJsonPath = path.join(process.cwd(), 'docs', 'status', 'reliability-monthly.latest.json')
const reportMdPath = path.join(process.cwd(), 'docs', 'status', 'reliability-monthly.latest.md')
...
async function gh(pathname) {
  ...
  const res = await fetch(`https://api.github.com/repos/${owner}/${repo}${pathname}`, {
    headers: {
      ...
      'User-Agent': 'startingmonday-reliability-monthly-trends',
    },
  })
...
function recommendationForTrend(workflow, trend) {
  ...
  if (trend.label === 'improving') {
    return `Issue rate improved by ${(Math.abs(trend.delta) * 100).toFixed(1)} points. Keep current reliability controls and document what worked.`
  }

  return 'Trend is stable. Continue weekly review and keep owning team on-call routing current.'
}

function buildMarkdown(report) {
  const lines = []
  lines.push('# Reliability Monthly Trends Report')
  lines.push('')
...
function buildSlackText(report) {
  const headline = report.summary.worse > 0
    ? `*Monthly reliability trends: ${report.summary.worse} workflow(s) worsened*`
    : '*Monthly reliability trends: no worsening workflows*'

  const trendLines = report.trends.map((row) => {
    return `- ${row.name}: ${row.trend.label} (current ${row.current.issueRate}, previous ${row.previous.issueRate})`
  })
...
async function main() {
  ...
  console.log('Reliability monthly trends report completed.')
}
