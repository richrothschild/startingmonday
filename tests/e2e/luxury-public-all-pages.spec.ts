...
import {
  LUXURY_PUBLIC_ROUTE_CONTRACTS,
  LUXURY_REDIRECT_ROUTE_CONTRACTS,
} from './route-contracts.manifest'
...
test.describe('Luxury + cognitive baseline for public pages @luxury', () => {
  for (const routeContract of LUXURY_PUBLIC_ROUTE_CONTRACTS) {
    test(`${routeContract.path} has premium hierarchy and bounded cognitive load`, async ({ page }) => {
      const route = routeContract.path
      const metrics = await getBaselineMetrics(page, `${BASE_URL}${route}`)
      const ctaLimit = routeContract.ctaLimit
      ...
      if (routeContract.requiresDisclosure) {
        expect(metrics.supportsDisclosure, `${route}: should include at least one details/summary disclosure`).toBeGreaterThanOrEqual(1)
      }
    })

    test(`${routeContract.path} has no horizontal overflow on luxury mobile`, async ({ page }, testInfo) => {
  ...
  for (const redirectContract of LUXURY_REDIRECT_ROUTE_CONTRACTS) {
    test(`${redirectContract.from} redirects to canonical route`, async ({ page }) => {
      const response = await page.goto(`${BASE_URL}${redirectContract.from}`, { waitUntil: 'domcontentloaded' })
      expect(response?.status() ?? 0).toBeLessThan(400)
      expect(page.url()).not.toBe(`${BASE_URL}${redirectContract.mustNotEndWith}`)
    })
  }
})
