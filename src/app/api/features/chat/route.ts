...
export async function POST(request: NextRequest) {
  ...
  if (!parsed.success) {
    return NextResponse.json({ error: 'Please ask a clear question with at least 3 characters.' }, { status: 400 })
  }

  const docs = await loadAllFeatureDocs()
  if (docs.length === 0) {
    return NextResponse.json({ error: 'Feature docs are temporarily unavailable.' }, { status: 503 })
  }
