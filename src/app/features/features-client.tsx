    ...
    <div className="min-h-screen bg-slate-100">
      ...
      <main className="mx-auto max-w-6xl px-4 py-8 sm:px-6 sm:py-10">
        <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
          ...
          <h1 className="mt-2 text-3xl font-bold tracking-tight text-slate-950">Feature and onboarding docs</h1>
          ...
          <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-3">
            <div className="rounded-xl border border-slate-200 bg-slate-50 p-3">
              <p className="text-[11px] uppercase tracking-[0.08em] text-slate-500">Documents</p>
              <p className="mt-1 text-xl font-semibold text-slate-900">{stats.totalDocs}</p>
            </div>
            <div className="rounded-xl border border-slate-200 bg-slate-50 p-3">
              <p className="text-[11px] uppercase tracking-[0.08em] text-slate-500">Total lines</p>
              <p className="mt-1 text-xl font-semibold text-slate-900">{stats.totalLines}</p>
            </div>
            <div className="rounded-xl border border-slate-200 bg-slate-50 p-3">
              <p className="text-[11px] uppercase tracking-[0.08em] text-slate-500">Headings audited</p>
              <p className="mt-1 text-xl font-semibold text-slate-900">{stats.totalHeadings}</p>
            </div>
          </div>
        </section>
        ...
        <section className="mt-5 rounded-2xl border border-slate-200 bg-white p-4 sm:p-5">
          <div className="grid grid-cols-1 gap-3 md:grid-cols-[1fr_auto] md:items-center">
            <input
              ...
              className="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-[14px] text-slate-900 placeholder:text-slate-400"
            />
            <select
              value={category}
              onChange={(event) => setCategory(event.target.value as typeof category)}
              aria-label="Filter by document category"
              className="rounded-lg border border-slate-300 bg-white px-3 py-2 text-[14px] text-slate-900"
            >
          ...
          <p className="mt-2 text-[12px] text-slate-500">Showing {filtered.length} of {docs.length} documents.</p>
        </section>
        ...
        <section className="mt-5 grid grid-cols-1 gap-3 lg:grid-cols-2">
          {filtered.map((doc) => (
            <article key={doc.slug} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
              <div className="flex flex-wrap items-center gap-2">
                <span className="rounded-full bg-slate-100 px-2 py-1 text-[11px] font-semibold text-slate-700">{CATEGORY_LABELS[doc.category]}</span>
                <span className="rounded-full bg-orange-50 px-2 py-1 text-[11px] font-semibold text-orange-700">{PERSONA_LABELS[doc.persona]}</span>
              </div>
              <h2 className="mt-2 text-lg font-bold text-slate-950">
                <Link href={`/features/${doc.slug}`} className="hover:text-slate-700 hover:underline">{doc.title}</Link>
              </h2>
              <p className="mt-1 text-[14px] leading-relaxed text-slate-600">{doc.summary}</p>
              <div className="mt-3 flex flex-wrap gap-3 text-[11px] text-slate-500">
                <span>{doc.lineCount} lines</span>
                <span>{doc.headingCount} headings</span>
                <span>Updated {formatDate(doc.updatedAt)}</span>
              </div>
              <p className="mt-2 text-[11px] text-slate-500">Last line: {doc.lastLine}</p>
              <div className="mt-3 flex flex-wrap items-center gap-3 text-[13px]">
                <Link href={`/features/${doc.slug}`} className="font-semibold text-slate-900 hover:text-slate-700 hover:underline">Open document</Link>
                {doc.landingHref ? <Link href={doc.landingHref} className="text-slate-600 hover:text-slate-800 hover:underline">Related page</Link> : null}
              </div>
            </article>
          ))}
        </section>
