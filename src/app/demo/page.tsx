...
export function DemoContent({
  ...
  const [lastRequest, setLastRequest] = useState<BriefRequestPayload | null>(null)
  ...
  async function generateBrief(targetCompany: string, targetRole: string) {
    ...
    if (loading) {
      setRequestStatus('Already generating. Please wait for this run to finish.')
      return
    }

    setLastRequest({ company: trimmedCompany, role: trimmedRole })
    setContent('')
    setError('')
    setRevealed(false)
    setLoading(true)
    setRequestStatus('Request sent. Building brief...')
    setFirstTokenSlow(false)

    const firstTokenTimer = setTimeout(() => {
      setFirstTokenSlow(true)
      setRequestStatus('Still working. Warming up the model...')
    }, 3000)
  ...
  async function handleRetryLastRequest() {
    ...
    await generateBrief(lastRequest.company, lastRequest.role)
  }
