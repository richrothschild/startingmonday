...
export function CultureFitRetentionSignalsCard() {
  return (
    <Card className="border-primary/30 bg-primary/5 p-5 sm:p-6">
      <p className="text-[12px] font-semibold uppercase tracking-[0.1em] text-primary">First version: culture-fit + retention signals</p>
      ...
      <div className="mt-4 grid gap-4 sm:grid-cols-2">
        <div>
          <p className="text-[12px] font-semibold uppercase tracking-[0.08em] text-muted-foreground">Retention signals</p>
          <ul className="mt-2 space-y-2">
            {RETENTION_SIGNALS.map((item) => (
              <li key={item.label} className="rounded-md border border-border bg-card/40 px-3 py-2.5">
                <p className="text-[13px] font-semibold text-foreground">{item.label}</p>
                <p className="mt-1 text-[12px] leading-relaxed text-muted-foreground">{item.why}</p>
              </li>
            ))}
          </ul>
        </div>
        ...
        <div>
          <p className="text-[12px] font-semibold uppercase tracking-[0.08em] text-muted-foreground">Culture-fit probe prompts</p>
          <ul className="mt-2 space-y-2">
            {CULTURE_FIT_PROMPTS.map((prompt) => (
              <li key={prompt} className="rounded-md border border-border bg-card/40 px-3 py-2.5 text-[13px] text-foreground">
                {prompt}
              </li>
            ))}
          </ul>
