...
export default async function DashboardPage() {
  ...
  const [{ data: companies }, { data: followUps }] = await Promise.all([
    supabase
      .from('companies')
      .select('id, name, sector, stage, fit_score, notes, updated_at')
      .eq('user_id', user.id)
      .is('archived_at', null)
  ...
  const stats = [
    { value: companyList.length, label: 'Companies', alert: false },
            ...
            <tbody>
              {companyList.length === 0 ? (
                ...
                return (
                  <tr
                    key={co.id}
                    className={i < companyList.length - 1 ? 'border-b border-slate-50' : ''}
                  >
