...
export const ExecutiveEmotionStateSchema = z.enum([
  'stable_momentum',
  'activation_friction',
  'threat_drift',
  'exhaustion_risk',
...
export type ExecutiveEmotionState = z.infer<typeof ExecutiveEmotionStateSchema>

export const ExecutiveEmotionConfidenceBandSchema = z.enum(['high', 'medium', 'low'])
export type ExecutiveEmotionConfidenceBand = z.infer<typeof ExecutiveEmotionConfidenceBandSchema>

export const ExecutiveInterventionModeSchema = z.enum(['optimize', 'stabilize', 'recover', 'decide'])
export type ExecutiveInterventionMode = z.infer<typeof ExecutiveInterventionModeSchema>

export const ExecutiveEmotionSignalWindowSchema = z.object({
  cadence_adherence_pct: z.number().min(0).max(100).optional(),
  followup_sla_miss_rate: z.number().min(0).max(1).optional(),
  rejection_streak_count: z.number().int().min(0).optional(),
  target_list_churn_rate: z.number().min(0).optional(),
  outreach_volume: z.number().int().min(0).optional(),
  outreach_specificity_score: z.number().min(0).max(100).optional(),
  sponsor_map_depth: z.number().min(0).optional(),
  recovery_started_48h: z.boolean().optional(),
  debrief_completion_rate: z.number().min(0).max(1).optional(),
  decision_lag_days: z.number().min(0).optional(),
  context_completion_rate: z.number().min(0).max(1).optional(),
  no_go_criteria_present: z.boolean().optional(),
  momentum_quality_index: z.number().min(0).max(100).optional(),
  strain_index: z.number().min(0).max(100).optional(),
  decision_load_index: z.number().min(0).max(100).optional(),
})
export type ExecutiveEmotionSignalWindow = z.infer<typeof ExecutiveEmotionSignalWindowSchema>

export const ExecutiveEmotionStateScoreRequestSchema = z.object({
  user_id: z.string().trim().min(1),
  stage: ExecutiveSearchStageSchema,
  archetype: z.string().trim().min(1).optional(),
  confidence_tier: ExecutiveConfidenceTierSchema.optional(),
  signal_window: ExecutiveEmotionSignalWindowSchema.optional().default({}),
})
export type ExecutiveEmotionStateScoreRequest = z.infer<typeof ExecutiveEmotionStateScoreRequestSchema>

export type ExecutiveEmotionStateScore = {
  state: ExecutiveEmotionState
  confidence: number
  confidence_band: ExecutiveEmotionConfidenceBand
  top_signal_drivers: string[]
  recommended_intervention_mode: ExecutiveInterventionMode
  fallback_applied: boolean
  evaluated_at: string
}

function toBand(confidence: number): ExecutiveEmotionConfidenceBand {
  if (confidence >= 0.75) return 'high'
  if (confidence >= 0.5) return 'medium'
  return 'low'
}

function modeForState(state: ExecutiveEmotionState): ExecutiveInterventionMode {
  if (state === 'stable_momentum') return 'optimize'
  if (state === 'rebound_recovery') return 'stabilize'
  if (state === 'decision_overload') return 'decide'
  return 'recover'
}
...
type StateScoreBucket = {
  state: ExecutiveEmotionState
  score: number
  drivers: string[]
}

export function scoreExecutiveEmotionState(input: ExecutiveEmotionStateScoreRequest): ExecutiveEmotionStateScore {
  const signals = input.signal_window ?? {}
  const providedSignals = Object.values(signals).filter((value) => value !== undefined).length
  const insufficientSignals = providedSignals < 3

  if (insufficientSignals) {
    const fallbackState: ExecutiveEmotionState = input.stage === 'OFFER_DECISION'
      ? 'decision_overload'
      : 'activation_friction'
    ...
    return {
      ...
      confidence: 0.42,
      confidence_band: 'low',
      top_signal_drivers: ['missing_signal_coverage'],
      recommended_intervention_mode: modeForState(fallbackState),
      fallback_applied: true,
      evaluated_at: new Date().toISOString(),
    }
  }

  const buckets: StateScoreBucket[] = [
    { state: 'stable_momentum', score: 0, drivers: [] },
    { state: 'activation_friction', score: 0, drivers: [] },
    { state: 'threat_drift', score: 0, drivers: [] },
    { state: 'exhaustion_risk', score: 0, drivers: [] },
    { state: 'decision_overload', score: input.stage === 'OFFER_DECISION' ? 0.4 : 0, drivers: [] },
    { state: 'rebound_recovery', score: 0, drivers: [] },
  ]

  const byState = Object.fromEntries(buckets.map((bucket) => [bucket.state, bucket])) as Record<ExecutiveEmotionState, StateScoreBucket>
  const add = (state: ExecutiveEmotionState, points: number, driver: string) => {
    byState[state].score += points
    byState[state].drivers.push(driver)
  }

  if ((signals.cadence_adherence_pct ?? 100) >= 80) add('stable_momentum', 1.4, 'cadence_adherence_strong')
  if ((signals.followup_sla_miss_rate ?? 1) < 0.2) add('stable_momentum', 1.0, 'followup_sla_controlled')
  if ((signals.momentum_quality_index ?? 0) >= 65) add('stable_momentum', 1.4, 'momentum_quality_high')
  if ((signals.rejection_streak_count ?? 0) === 0) add('stable_momentum', 0.8, 'no_active_rejection_streak')

  if ((signals.cadence_adherence_pct ?? 100) < 60) add('activation_friction', 1.4, 'cadence_adherence_low')
  if ((signals.target_list_churn_rate ?? 0) > 0.6) add('activation_friction', 1.0, 'target_churn_high')
  if ((signals.outreach_volume ?? 0) < 3) add('activation_friction', 0.9, 'outreach_volume_low')
  if ((signals.outreach_specificity_score ?? 100) < 50) add('activation_friction', 0.7, 'message_specificity_low')

  if ((signals.outreach_specificity_score ?? 100) < 45) add('threat_drift', 1.2, 'outreach_specificity_degrading')
  if ((signals.sponsor_map_depth ?? 99) < 2) add('threat_drift', 0.9, 'sponsor_depth_shallow')
  if ((signals.rejection_streak_count ?? 0) >= 2) add('threat_drift', 0.8, 'rejection_streak_present')
  if ((signals.outreach_volume ?? 99) < 2) add('threat_drift', 0.7, 'outreach_withdrawal_detected')

  if ((signals.rejection_streak_count ?? 0) >= 3) add('exhaustion_risk', 1.3, 'rejection_streak_escalating')
  if ((signals.debrief_completion_rate ?? 1) < 0.4) add('exhaustion_risk', 0.8, 'debrief_completion_low')
  if ((signals.momentum_quality_index ?? 100) < 45) add('exhaustion_risk', 1.2, 'momentum_quality_decline')
  if ((signals.strain_index ?? 0) > 60) add('exhaustion_risk', 1.3, 'strain_index_high')
  if (signals.recovery_started_48h === false) add('exhaustion_risk', 0.7, 'recovery_not_started_48h')

  if ((signals.decision_lag_days ?? 0) > 10) add('decision_overload', 1.4, 'decision_lag_high')
  if ((signals.context_completion_rate ?? 1) < 0.7) add('decision_overload', 1.0, 'context_capture_incomplete')
  if (signals.no_go_criteria_present === false) add('decision_overload', 0.9, 'no_go_criteria_missing')
  if ((signals.decision_load_index ?? 0) > 60) add('decision_overload', 1.2, 'decision_load_elevated')

  if (signals.recovery_started_48h === true) add('rebound_recovery', 1.4, 'recovery_started_48h')
  if ((signals.outreach_specificity_score ?? 0) >= 55) add('rebound_recovery', 0.8, 'message_specificity_recovering')
  if ((signals.cadence_adherence_pct ?? 0) >= 65) add('rebound_recovery', 0.9, 'cadence_recovering')
  if ((signals.rejection_streak_count ?? 99) <= 1) add('rebound_recovery', 0.8, 'rejection_streak_controlled')

  const ranked = [...buckets].sort((a, b) => b.score - a.score)
  const winner = ranked[0]
  const runnerUp = ranked[1]
  const confidence = roundToTwo(Math.min(0.95, 0.45 + ((winner.score - runnerUp.score) * 0.1) + (providedSignals * 0.02)))

  return {
    state: winner.state,
    confidence,
    confidence_band: toBand(confidence),
    top_signal_drivers: winner.drivers.slice(0, 3),
    recommended_intervention_mode: modeForState(winner.state),
