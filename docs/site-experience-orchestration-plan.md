...
| J8 Journey Synthetic Agent         | In Progress | `.github/workflows/production-synthetics.yml` now emits journey step percentile and abandonment-risk scoring (`synthetic-journey-metrics.json`) with SES-driven thresholds and route-tier weighting                                                                                | Add journey metric normalization into portfolio rollup issue clustering                                                                                                                            |
...
| J10 Weekly Issues + Monthly Trends | In Progress | `scripts/experience-weekly-issues-report.mjs`, `scripts/experience-monthly-trends-report.mjs`, `scripts/trust-weekly-issues-report.mjs`, `scripts/trust-monthly-trends-report.mjs`, `scripts/experience-portfolio-rollup.mjs`, `.github/workflows/experience-portfolio-rollup.yml` | Monthly trends now classify source-staleness directionality (improving/flat/worsening) with owner deltas; next feed this directionality into monthly action prioritization and escalation copy     |
| J11 Watchdog + seeding             | In Progress | `.github/workflows/monitoring-watchdog.yml`, `.github/workflows/experience-seeding-checklist.yml`, `scripts/experience-seeding-checklist.mjs`, `.github/workflows/probe-account-reset.yml`, `scripts/reset-probe-account.mjs`, `scripts/experience-weekly-issues-report.mjs`       | Weekly report now links seeding checklist and probe-reset artifacts, and watchdog emits a J11 freshness signal summary; next tighten the freshness windows after one more weekly cycle             |
| J12 Calibration loop               | Done        | `scripts/cognitive-calibration-loop.mjs`, `.github/workflows/cognitive-calibration-loop.yml`, `.github/workflows/cognitive-fluency-auditor.yml`                                                                                                                                    | Page Experience Auditor artifact ingestion and workflow-native publication wired into automated route-level grade comparison; quarterly calibration produces deterministic-vs-auditor grade deltas |
...
> We have world-class _instruments_ but no _observatory_. Gates prevent regressions at commit time; nothing continuously verifies that the **live rendered experience** still meets Page Experience Auditor expectations, and nothing accumulates experience data over time the way the reliability suite now does for workflows.
